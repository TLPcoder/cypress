...
class Tasks {
     ...
     * @param {Object=} tasks.serial tasks that need to be run one after another
     * @param {String=} method http 
     * @returns {Promise}
     */

    async runTasks(ctx, tasks, method) {
        const { parallel, serial } =  tasks
        return Promise.all(
            [
                this.parallel(ctx, await this.resolveTask(ctx, parallel, method)),
                this.serial(ctx, await this.resolveTask(ctx, serial, method))
            ]
        )
    }
    ...
    resolveTask(ctx, tasks = [], method) {
        return Promise.all(tasks.map(async task => {
            ...
            if (this.utils.checkMethod(method, task.methods) || this.type === 'middleware') {
                return require(path.normalize(`${this.utils.getConfigPath()}/${task.middlewarePath}`))
            }
